...
public class AnalyzerSettings
{
    ...
    // ── Credentials ──────────────────────────────────────────────────────
    /// <summary>True = použít aktuální Windows identity. False = explicitní credentials.</summary>
    public bool UseIntegratedSecurity { get; set; } = true;
    /// <summary>Uživatelské jméno (DOMAIN\user nebo user@domain).</summary>
    public string Username { get; set; } = "";
    /// <summary>Heslo — jen v paměti, neukládá se do JSON.</summary>
    public string Password { get; set; } = "";

    /// <summary>Parsuje domain a username z formátu DOMAIN\user nebo user@domain.</summary>
    ...
    public string ConnectionString
    {
        get
        {
            var sb = $"Server={SqlServer};Database={DatabaseName};TrustServerCertificate=true;" +
                     "Connect Timeout=30;Command Timeout=120;";
            if (UseIntegratedSecurity || string.IsNullOrEmpty(Username))
                return sb + "Integrated Security=true;";
            else
                return sb + $"User ID={Username};Password={Password};";
        }
    }
