...
/// <summary>READ-ONLY: Čte BC Slow SQL eventy z Application Event Logu vzdáleného serveru.</summary>
public class EventLogCollector
{
    ...
    public List<SlowSqlEvent> Collect(CancellationToken ct = default)
    {
        ...
        var authMode = _cfg.UseIntegratedSecurity ? "Integrated Security" : $"Explicit ({_cfg.Username})";
        _log.Info($"Event Log: {_cfg.NavServer} | {_cfg.EventSource} | ID {_cfg.EventId} | {_cfg.LookbackHours}h | Auth: {authMode}");

        try
        {
            EventLogSession session;

            if (_cfg.UseIntegratedSecurity || string.IsNullOrEmpty(_cfg.Username))
            {
                // Integrated Security — aktuální Windows identity
                session = new EventLogSession(_cfg.NavServer);
                _log.Info("Event Log: Připojuji se s aktuálním Windows účtem...");
            }
            else
            {
                // Explicitní credentials
                var (domain, user) = _cfg.ParseCredentials();
                var secPwd = new SecureString();
                foreach (var c in _cfg.Password) secPwd.AppendChar(c);
                secPwd.MakeReadOnly();
                ...
                _log.Info($"Event Log: Připojuji se jako {domain}\\{user}...");
            }
        ...
        catch (UnauthorizedAccessException ex)
        {
            ...
            _log.Error("Tip: Ověřte, že účet má oprávnění 'Event Log Readers' na vzdáleném serveru.");
            throw;
        }
