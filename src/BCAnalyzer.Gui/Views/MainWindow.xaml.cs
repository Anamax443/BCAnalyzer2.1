...
public partial class MainWindow : Window
{
    ...
    {
        if (PnlCredentials == null) return; // XAML ještě není loaded
    ...
    private async void BtnTestConn_Click(object sender, RoutedEventArgs e)
    {
        ...
        await Task.Run(() =>
        {
            ...
            try
            {
                ...
                if (rdr.Read())
                    results.Add($"✅ SQL Server: {rdr.GetString(0)} — připojení OK");
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                {
                    session = new EventLogSession(cfg.NavServer);
                }
                else
                {
                    var (domain, user) = cfg.ParseCredentials();
                    var secPwd = new SecureString();
                    foreach (var c in cfg.Password) secPwd.AppendChar(c);
                    secPwd.MakeReadOnly();
                    session = new EventLogSession(cfg.NavServer, domain, user, secPwd, SessionAuthentication.Default);
                }

                // Zkusíme přečíst 1 event jako test
                var xpath = $"*[System[Provider[@Name='{cfg.EventSource}'] and (EventID={cfg.EventId})]]";
                var query = new EventLogQuery(cfg.EventLogName, PathType.LogName, xpath)
                {
                    Session = session,
            ...
            catch (UnauthorizedAccessException ex)
            {
                ...
                results.Add("   Tip: Účet potřebuje oprávnění 'Event Log Readers' na vzdáleném serveru.");
            }
            catch (Exception ex)
            {
